...
padronizar_texto(df_merge.at[0,'text'])
...
data_movie[data_movie['Movie name'] == "Table for Five"]
...
df_teste = pd.DataFrame(data = {"Previsão":multilabel_binarizer.inverse_transform(y_pred), 
                                "Teste":multilabel_binarizer.inverse_transform(y_test)})
df_teste
...
# split dataset into training and validation set
x_train, x_test, y_train, y_test = train_test_split(df_merge['text'][:1000], y[:1000], test_size = 0.2, random_state = 42)
...
# criar um modelo de Random Forest com 100 árvores
...
# Avaliar a acurácia do modelo a partir das previsões nos testes
